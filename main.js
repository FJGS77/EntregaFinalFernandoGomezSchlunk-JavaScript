Swal.fire({
    title: "Bienvenidos a LA CALCU!",
    text: "Creado por: Fernando Gómez Schlunk",
    width: 700,
    padding: "5em",
    color: "wheat",
    background: "#fff url(/assets/images/alert.webp) bottom",
});

fetch("./data.json")
    .then(response => response.json())
    .then(data => {
        const impGanancias = data.impGanancias;
        const bienesPersonales = data.bienesPersonales;
        const impPais = data.impPais;
        const iva21 = data.iva21;
        const iva105 = data.iva105;
        const iva27 = data.iva27;
        ...
        function asignarCalculadora(opcion) {
            if (opcion === 1) {
                ...
                function crearCalculadora(contenedor) {
                    ...
                    document.addEventListener("keydown", function (event) {
                        ...
                        if (teclaPresionada === "Enter") {
                            event.preventDefault(); cliquearBoton("=");
                        } else if (teclaPresionada === "Delete" || teclaPresionada === "Backspace") {
                            event.preventDefault();
                            cliquearBoton("DEL");
                        } else {
                            const botonCorrespondiente = botones.find(boton => boton.label === teclaPresionada);
                            if (botonCorrespondiente) {
                                cliquearBoton(botonCorrespondiente.label);
                            }
                        }
                    });
                }
                ...
                const importeGuardado = parseFloat(localStorage.getItem("userImporte")) || 0;
                ...
                function calcularImpuestos() {
                    ...
                    mostrarResultado("Impuesto País (17,5%)", resultadoImpPais);
                ...
                nombresServicios.forEach(nombre => {
                    ...
                    servicioBtn.addEventListener('mouseover', function () {
                        this.style.cursor = 'pointer';
                    });
